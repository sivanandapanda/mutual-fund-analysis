...
public class LocalSearchService {
    ...
    private void saveAll(List<MfMetaData> mutualFundList) {
        Map<String, List<MfMetaData>> cache = new HashMap<>();
        counter.set(0);
        ...
        mutualFundList.parallelStream().forEach(mf -> {
            ...
            getAllTags(mf.getSchemeName())
                    .forEach(tag -> {
                        List<MfMetaData> list = cache.getOrDefault(tag, new ArrayList<>());
                        list.add(mf);
                        cache.put(tag, list);
                    });
        });
